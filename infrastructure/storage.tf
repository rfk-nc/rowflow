module "storage" {
  ...
  log_analytics_workspace_id                              = module.log_analytics_workspace[local.primary_region].id
  monitor_diagnostic_setting_storage_account_enabled_logs = var.features.diagnostics_enabled ? local.monitor_diagnostic_setting_storage_account_enabled_logs : null
  monitor_diagnostic_setting_storage_account_metrics      = var.features.diagnostics_enabled ? local.monitor_diagnostic_setting_storage_account_metrics : null
...
# Private Endpoint Configuration if enabled
  private_endpoint_properties = var.features.private_endpoints_enabled ? {
    private_dns_zone_ids_blob            = module.private_dns_zones["${each.value.region_key}-storage_blob"].private_dns_zone_ids
    private_dns_zone_ids_table           = module.private_dns_zones["${each.value.region_key}-storage_table"].private_dns_zone_ids
    private_dns_zone_ids_queue           = module.private_dns_zones["${each.value.region_key}-storage_queue"].private_dns_zone_ids
