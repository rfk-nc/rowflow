module "container-app-worker" {
  ...
  acr_managed_identity_id = each.value.container_registry_use_mi ? module.acr[0].mi_id : null
  docker_image            = "${module.acr[0].login_server}/${each.value.docker_image}:${each.value.docker_env_tag}"
